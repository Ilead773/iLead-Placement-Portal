...
# 1. Read the Cleaned 7th Sem Details CSV
...
print(f"Total students in Excel sheet: {len(df)}")
...
# 3. Find students in CSV who DID NOT receive a Welcome Email
missed_in_csv = []
for idx, row in df.iterrows():
    email = str(row['Email ID']).lower().strip()
    if email not in sent_set:
        missed_in_csv.append({
            "name": row['Student Name'],
            "email": email,
            "roll": str(row['Roll No.']) if 'Roll No.' in row else ''
        })

print(f"Total students in Excel who MISSED welcome email: {len(missed_in_csv)}")
...
# Map missed students from CSV by email
missed_emails_set = {m["email"] for m in missed_in_csv}
