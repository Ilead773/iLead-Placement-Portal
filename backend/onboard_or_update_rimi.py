...
reg_no = '28999625003'
target_email = 'rimid554@gmail.com'
target_name = 'Rimi Das'
target_course = 'MSc Media Science'
target_semester = 3

print("=== STARTING RIMI DAS ONBOARDING/UPDATE ===", flush=True)

student = Student.objects.filter(registration_number=reg_no).first()
if not student:
    student = Student.objects.filter(email__iexact=target_email).first()

if not student:
    print(f"Student {target_name} not found in DB. Creating new Student and User records...", flush=True)
    ...
    print(f"Found existing Student: {student.name} ({student.registration_number})", flush=True)
...
print(f"Generated Temp Password: {temp_password}", flush=True)
...
print(f"✅ Welcome email successfully sent synchronously via Brevo backend to {target_email}!", flush=True)

# Append details in Sent_Emails_Updated.xlsx
...
if os.path.exists(file_path):
    ...
    print("Successfully appended record to Sent_Emails_Updated.xlsx!", flush=True)
