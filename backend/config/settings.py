...
ALLOWED_HOSTS = [host.strip().strip('"').strip("'") for host in os.environ.get('ALLOWED_HOSTS', 'localhost,127.0.0.1,0.0.0.0').split(',') if host.strip()]
...
# Configure Django default file storage based on backend choice
...
_raw_senders = os.environ.get('BREVO_SENDER_OPTIONS', '').strip('\'"')
if _raw_senders:
...
_raw_rotation = os.environ.get('BREVO_ROTATION_CONFIG', '').strip('\'"')
if _raw_rotation:
...
NORTH_STAR_MIN_ATTENDANCE_PERCENT = float(os.getenv('NORTH_STAR_MIN_ATTENDANCE_PERCENT', '75.0'))
