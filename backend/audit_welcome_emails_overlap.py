...
print("=== AUDITING WELCOME EMAIL OVERLAP (AUG 24 WELCOME EMAILS VS FORM COMPLAINTS) ===", flush=True)

try:
    conn = psycopg2.connect(DATABASE_URL)
    cur = conn.cursor(cursor_factory=RealDictCursor)
    
    # 1. Pull all email addresses that received a welcome email yesterday (Aug 24)
    # The SentEmailLog table typically has subject = 'Welcome to iLEAD Placement Portal - Account Created'
    # Or subject starts with 'Welcome to iLEAD Placement'
    cur.execute("""
        SELECT recipient, sent_at 
        FROM core_sentemaillog 
        WHERE sent_at::date = '2026-08-24' 
          AND subject LIKE '%%Welcome%%'
    """)
    sent_emails = cur.fetchall()
    sent_set = {s["recipient"].lower().strip() for s in sent_emails}
    
    print(f"Total welcome emails logged yesterday: {len(sent_emails)}", flush=True)
    
    # 2. Compare the 45 Google Form complaining students with the sent emails set
    overlap_sent = []
    not_sent = []
    
    for c in new_complaints:
        form_email = c["email"].lower().strip()
        # Find if student has record in DB to get DB registered email
        cur.execute("SELECT email FROM students WHERE registration_number = %s", (c["roll"],))
        s_rec = cur.fetchone()
        
        db_email = s_rec["email"].lower().strip() if s_rec else None
        
        # Check both the Form email and the DB email
        was_sent = False
        matched_email = None
    ...
    print(f"\n--- COMPLAINING STUDENTS WHO WERE SENT EMAILS YESTERDAY ({len(overlap_sent)}) ---", flush=True)
    for o in overlap_sent:
        print(f"  - {o['name']:25} | Roll: {o['roll']:12} | Sent to: {o['email']}", flush=True)
        
    print(f"\n--- COMPLAINING STUDENTS WHO WERE NOT SENT EMAILS YESTERDAY ({len(not_sent)}) ---", flush=True)
    for n in not_sent:
        reg_status = "Registered" if n["registered"] else "NOT Registered"
        print(f"  - {n['name']:25} | Roll: {n['roll']:12} | Status: {reg_status} | Email: {n['email']}", flush=True)
