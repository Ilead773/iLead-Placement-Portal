import os, django, secrets, string, time
...
print("=== NEW BATCH RESILIENT PASSWORD RESET (DATABASE ONLY) ===")
...
for c in new_complaints:
    ...
    while retries > 0:
        try:
            ...
            retries -= 1
            print(f"Database error during {c['name']}, retrying in 2 seconds... (Retries left: {retries})")
            time.sleep(2)
            connection.close() # Force close connection to reconnect

print("-" * 95)
print(f"Successfully reset credentials for {reset_count} students in DB.")
